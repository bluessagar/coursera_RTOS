...
void main(void)
{
    struct utsname buf;
    uname(&buf);
    syslog(LOG_INFO,"[COURSE:1][ASSIGNMENT:4] %s %s %s %s %s GNU/Linux",buf.sysname,buf.nodename,buf.release,buf.version,buf.machine);
...
void *Service_3(void *threadp)
{
    ...
    while(!abortS3)
    {
        ...
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "S3 6.66 Hz on core %d forrelease %llu @ sec=%6.9lf\n", sched_getcpu(), S3Cnt, current_realtime-start_realtime);
    }
