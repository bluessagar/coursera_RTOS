...
/* Updated by Sagar Dabhole as part of the course assignment.               */
/****************************************************************************/
...
// support function to get time dofferance 
...
// support function to calculate the time differance. 
...
// support function to test the delay.
...
// support function to end the delay test.
void end_delay_test(void)
{
    double real_dt;
#if 0
...
void main(void)
{
   ...
   // print the scheduller policy before adjustment
   printf("Before adjustments to scheduling policy:\n");
   print_scheduler();

  syslog(LOG_INFO, "[COURSE:1][ASSIGNMENT:3]: Before Main thread running on CPU=%d\n", sched_getcpu());

#ifdef RUN_RT_THREAD

  // setup the main thread as a RT thread with max priority, SCHED_FIFO policy
   ...
   // print the scheduller policy after adjustment
   printf("After adjustments to scheduling policy:\n");
   print_scheduler();
   syslog(LOG_INFO, "[COURSE:1][ASSIGNMENT:3]: After Main thread running on CPU=%d\n", sched_getcpu());
