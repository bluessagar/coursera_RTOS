...
// function to print current scheduler
...
// fun to calculate diff between 2 timestamps in double
...
// fun to calculate time diff between 2 timestamps as timespec
...
void *delay_test(void *threadID)
{
  ...
  // iterate and calculate the time stamp differeances
...
void end_delay_test(void)
{
  ...
  syslog(LOG_INFO, "MY_CLOCK clock DT seconds = %ld, msec=%ld, usec=%ld, nsec=%ld, sec=%6.9lf\n", 
...
#endif
  syslog(LOG_INFO, "MY_CLOCK delay error = %ld, nanoseconds = %ld\n", 
...
void main(void)
{
   ...
   syslog(LOG_INFO, "Before adjustments to scheduling policy:\n");
   ...
   syslog(LOG_INFO, "After adjustments to scheduling policy:\n");
